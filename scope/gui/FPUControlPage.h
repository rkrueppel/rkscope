...
public:
	...
	* be updated accordingly. The FOVDiagram shows all areas and thus needs information about all areas. */
	CFPUControlPage(const uint32_t& _area, std::vector<std::unique_ptr<parameters::BaseArea>>& _allareas, FPUButtons& _fpubuttons, const double& _masterfovsizex, const double& _masterfovsizey);
