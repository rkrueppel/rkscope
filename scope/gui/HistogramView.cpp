...
namespace scope {
	namespace gui {
		...
		CHistogramView::CHistogramView(const uint32_t& _area, const uint32_t& _channels, const uint16_t& _range, DisplayController& _display_controller)
			: area(_area)
			, channels(_channels)
		...
		void CHistogramView::OnMouseMove(UINT nFlags, CPoint point) {
			if ( nFlags == MK_LBUTTON ) {
				for ( uint32_t c = 0 ; c < channels ; c++ ) {
					...
					display_controller.SetHistogramLimits(area, c, lower_limits[c], upper_limits[c]);
				}
				Invalidate();
		...
		void CHistogramView::FullRange() {
			...
			range = static_cast<uint16_t>(range);
			for ( uint32_t c = 0 ; c < channels ; c++ ) {
				lower_limits[c] = 0;
