...
/// @file Taken from: http://www.codeproject.com/Articles/16737/WTL-Toolbar-Helper
/// Added support for ComboBoxEx, RK 2013-04-02

#include "gui\controls\ScopeColorComboCtrl.h"
...
public:
	...
	/// Create a comboboxex on a toolbar
	HWND CreateToolbarComboBoxEx(HWND hWndToolBar, UINT nID, UINT nWidth = 16, UINT nHeight = 16,
				   DWORD dwComboStyle = CBS_DROPDOWNLIST)
	{
		T* pT = static_cast<T*>(this);
		// Use built-in WTL toolbar wrapper class
		...
		combo.SetFont(m_fontCombo);
		combo.SetParent(toolbar);
		combo.SetExtendedStyle(NULL, CBES_EX_TEXTENDELLIPSIS);
		// The combobox might not be centred vertically, and we won't know the
		// height until it has been created.  Get the size now and see if it
		// needs to be moved.
		CRect rectToolBar;
