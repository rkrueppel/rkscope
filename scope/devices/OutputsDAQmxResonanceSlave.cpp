...
int32_t OutputsDAQmxResonanceSlave::Write(const std::vector<int16_t>& _zp, const uint32_t& _blocks) {
	int32_t written = 0;
	assert(_blocks!=0);
...
ZeroOutputsDAQmxResonanceSlave::ZeroOutputsDAQmxResonanceSlave(const parameters::OutputsDAQmxResonanceSlave& _params) {
	...
	try {
		...
		task.CreateAOVoltageChannel(_params.zpout()
			...
			, -_params.range()
			, _params.range());
		task.WriteAnalogI16(std::make_shared<std::vector<int16_t>>(2,0)->data(), 1, true);
