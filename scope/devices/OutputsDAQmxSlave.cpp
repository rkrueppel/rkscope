...
OutputsDAQmxSlave::OutputsDAQmxSlave(const uint32_t& _area, const parameters::OutputsDAQmxSlave& _outputparams, const parameters::Scope& _params)
	...
	int32_t pixelsperchan = _params.slaveareas[area].Currentframe().XTotalPixels();

	if ( _params.requested_mode() == DaqModeHelper::nframes )
		pixelsperchan = _params.slaveareas[area].Currentframe().TotalPixels() * _params.slaveareas[area].daq.requested_frames() * _params.slaveareas[area].daq.averages();
	...
	zpout_task.ConfigureBuffer(_params.slaveareas[area].Currentframe().XTotalPixels());
