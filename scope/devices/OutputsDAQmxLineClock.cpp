...
OutputsDAQmxLineClock::OutputsDAQmxLineClock(const uint32_t& _area, const parameters::OutputsDAQmxLineClock& _outputparams, const parameters::Scope& _params)
	: Outputs(_area)
	, XTotalPixels(_params.allareas[area]->Currentframe().XTotalPixels())
	, YTotalLines(_params.allareas[area]->Currentframe().YTotalLines()) {
	...
	yzout_task.ConfigureSampleTiming(yzclockstring, _params.allareas[area]->linerate(), YTotalLines, DAQmx_Val_ContSamps, DAQmx_Val_Rising);
