...
namespace scope {
	...
	class Inputs {
		...
		public:
			...
			Inputs(const uint32_t& _masterarea);
			...
			* @return the number of samples per channel actually read */
			virtual int32_t Read(DaqMultiChunk<2, 1, uint16_t>& _chunk, bool& _timedout, const double& _timeout) {
				return -1;
			}
			...
			* @param[in] _timeout timeout in seconds after which to return even if not enough samples were read
			* @return the number of samples per channel actually read */
			virtual int32_t Read(DaqMultiChunk<2, 2, uint16_t>& _chunk, bool& _timedout, const double& _timeout) {
				return -1;
			}
	};
